        ...
        if (commonType.is('array-like') || commonType.is('object')) {
            ...
            return expect.promise.all([
                ...
                return expect.promise.settle(promiseByKey).then(function () {
                    function wrapArrayItemsInObjectsWithIndex(arr) {
                        return Array.prototype.map.call(arr, function (_, i) {
                            return { index: i, source: arr };
                        });
                    }

                    if (commonType.is('array-like')) {
                        var satisfyMatrix = Array.prototype.map.call(subject, function (subjectItem, i) {
                            return Array.prototype.map.call(value, function (valueItem, j) {
                                return expect.promise(function () {
                                    return expect(subject[i], 'to satisfy', value[j]);
                                });
                            });
                        });
                        return expect.promise.settle(satisfyMatrix).then(function () {
                            expect.fail({
                                diff: function (output, diff, inspect, equal) {
                                    var changes = require('array-changes')(wrapArrayItemsInObjectsWithIndex(subject), wrapArrayItemsInObjectsWithIndex(value), function (subjectIndex, valueIndex) {
                                        return satisfyMatrix[subjectIndex.index][valueIndex.index].isFulfilled();
                                    });
                                    ...
                                    changes.forEach(function (diffItem, index) {
                                        var diffItemValue = diffItem.value.source === subject ? subject[diffItem.value.index] : value[diffItem.value.index];
                                        var delimiterOutput = subjectType.delimiter(output.clone(), index, indexOfLastNonInsert + 1);
                                        output.i().block(function () {
                                            var type = diffItem.type;
                                            if (type === 'insert') {
                                                this.annotationBlock(function () {
                                                    this.error('missing ').block(inspect(diffItemValue));
                                                });
                                            } else if (type === 'remove') {
                                                this.block(inspect(diffItemValue).amend(delimiterOutput.sp()).error('// should be removed'));
                                            } else if (type === 'equal') {
                                                this.block(inspect(diffItemValue).amend(delimiterOutput));
                                            } else {
                                                var promise = satisfyMatrix[diffItem.value.index][diffItem.expected.index];
                                                var error = promise.reason();
                                                error.errorMode = 'bubble';
                                                var valueDiff = error.getDiff(output);
                                                if (valueDiff && valueDiff.inline) {
                                                    this.block(valueDiff.diff.amend(delimiterOutput));
                                                } else {
                                                    this.block(inspect(diffItemValue).amend(delimiterOutput.sp())).annotationBlock(function () {
                                                        this.appendErrorMessage(error, { omitSubject: diffItemValue });
                                                    });
                                                }
                                            }
                                        }).nl();
