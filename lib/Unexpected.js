...
Unexpected.prototype.lookupAssertionRule = function (subject, testDescriptionString, args) {
    ...
    for (var subjectTypeIndex = 0 ; subjectTypeIndex < that.types.length ; subjectTypeIndex += 1) {
        var candidateType = that.types[subjectTypeIndex];
        if (candidateType.identify && candidateType.identify(subject)) {
            var subjectType = candidateType;
